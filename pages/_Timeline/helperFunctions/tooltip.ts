...
export default function tooltips(chart: any) {
  ...
  const tooltipSelection = wrap.selectAll('.hover').data([1]).join('g').attr('class', 'hover');

  const hover = (tooltip: any, pos: any, text: any) => {
    const sidePadding = 5;
    ...
    tooltip
      ...
      .text((d: any) => d)
      .attr('y', (d: any, i: any) => (i - (text.length - 1)) * 15 - verticalOffset)
  ...
  wrap.selectAll('title').each(function () {
    ...
    const title: any = d3.select(this);
    // @ts-ignore
    ...
    parentNode
      .on('pointerenter pointermove', function (event) {
        ...
        if (text) tooltipSelection.call(hover, pointer, text.split('\n'));
        else tooltipSelection.selectAll('*').remove();
        ...
        // @ts-ignore
        const tipSize = tooltipSelection.node().getBBox();
        // Left side logic
        if (pointer[0] + tipSize.x < 0)
          tooltipSelection.attr('transform', `translate(${tipSize.width / 2}, ${pointer[1] + 7})`);
        // Right side logic
        else if (pointer[0] + tipSize.width / 2 > Number(wrap.attr('width')))
          tooltipSelection.attr(
            'transform',
            `translate(${Number(wrap.attr('width')) - tipSize.width / 2}, ${pointer[1] + 7})`
          );
      })
      .on('pointerout', () => {
