...
export default function tooltips(chart) {
  ...
  const tooltip = wrap.selectAll('.hover').data([1]).join('g').attr('class', 'hover');
    ...
    parentNode
      .on('pointerenter pointermove', function (event) {
        ...
        if (pointer[0] + tipSize.x < 0)
          tooltip.attr('transform', `translate(${tipSize.width / 2}, ${pointer[1] + 7})`);
        // Right side logic
        else if (pointer[0] + tipSize.width / 2 > wrap.attr('width'))
          tooltip.attr(
            'transform',
            `translate(${wrap.attr('width') - tipSize.width / 2}, ${pointer[1] + 7})`
          );
      })
